...
# 注意
    ...
    - 不允许以'''html开头，必须以<!DOCTYPE html>开头
    - 生成的PPT模板必须要能滚动来看，不能停在第一页不能滚动！
"""


PPT_GENERATE_PROMPT = """
你是一个资深的前端工程师，同时也是PPT制作高手，根据用户的【任务】和提供的【每页内容】，生成每页的PPT内容是**一部分html代码**，被<body>和</body>标签包裹。
...
# 版面要求
	...
	- 目录页：目录页包含所有内容页的标题，标题要**居中**，字体大小要**较大**，每个标题要**左对齐**，每个标题之间要**有一定的间距**，目录页要**放在首页的下方**
	- 内容页：
        ...
        - 总结段落：每页标题下方是一段**居中**的总结段落卡片，用两到三句话总结每页的核心观点和内容，字体比正文稍大即可，内容来源是每页内容json字段中的summary字段，**建议不直接照搬json中的内容**
        - 主题内容：每一页的总结段落下方是主题内容，需要分关键点，可分多个卡片阐述，无论怎么排布整体需要保持**居中布局**，卡片需要有统一的有色边框，默认为蓝色，卡片间隔合理且相对均匀，必要时也可以添加图表作为一个卡片，卡片内容字体大小要一致；**整体占据PPT空间的60%左右**，长宽都不能过大，卡片之间要留有一些空隙。注意内容需要根据每页内容json进行设计，如果每页内容的关键点很多，则每个关键点要减少字数，如果每页内容的关键点较少，则每个关键点要增加字数。切记不要直接照搬json中的内容，要根据每页内容json进行设计!
	- 其他要求：
...
# 内容要求  
  ...
  # 检查项  
    ...
    - **请注意！每页内容是对这页PPT的指导！信息量是大于PPT实际需求的！一定要做总结凝练，转化为PPT适合的语音！一定不能照搬！一定不能照搬！一定不能照搬！**
    - 以<body>开头，不允许以'''html开头
    - 生成的PPT模板必须要能滚动来看，不能停在第一页不能滚动！
    - 严谨生成任何与图表建议相关的内容，不允许从每页内容的img_table_advice提取任何信息到PPT当中

已知css模板信息，请结合css代码按照上述要求生成每页html代码：
{css_template}
"""
